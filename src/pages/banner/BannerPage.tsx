...
import type { IBannerData } from "./BannerValidator";
import { useEffect, useState } from "react";
import bannerService from "../../services/brand.service";
...
const BannerPage = () => {
    ...
    const [selectedBanner, setSelectedBanner] = useState<IBannerData | null>(null);
    ...
    const handleDelete = async (bannerId: string) => {
        ...
        try {
            ...
            toast.success('Banner deleted successfully!', {
                description: 'The selected banner has been successfully deleted.',
            });
            await getBanners({ page: PaginationDefault.page, limit: PaginationDefault.limit })
        } catch {
            toast.error('Failed to delete banner:', {
                description: 'An error occurred while deleting the banner. Please try again.',
            });
        }
        setOpen(false);
    ...
    const columns = [
        { title: 'Image', dataIndex: 'image', key: 'image', render: (image: string) => <img src={image} alt="Banner" style={{ width: '50px', height: '50px' }} /> },
        ...
        {
            title: 'Actions', dataIndex: '_id', key: 'actions', render: (id: string, data: IBannerData) => {
                return (
                    <>
                        <NavLink to={'/admin/banners/' + id}>
                            <Button type="primary" className="!bg-indigo-500 hover:!bg-indigo-600 items-center p-3! h-9! mr-2"><EditFilled/></Button>
                        </NavLink>
                        <Button type="primary" danger className="items-center p-3! h-9!" onClick={() => { setOpen(true); setSelectedBanner(data); }}><DeleteFilled /></Button>
                    </>
                )
            }
        }
    ...
    return (
        <>
            {loading && (
                <div style={{
                    ...
                    <div style={{
                        ...
                        <span style={{ fontSize: 20, color: '#fff', fontFamily: 'Poppins, sans-serif', fontWeight: 500 }}>Loading banners...</span>
                    </div>
                    <style>{`
                        .dual-ring-spinner {
            ...
            <div className="p-7 flex flex-col" style={{
                ...
                <div>
                    <h2 className="text-2xl font-poppins! font-semibold mb-2">Banners</h2>
                    <Breadcrumb
                    style={{
                        margin: '16px 0',
                        fontFamily: 'Poppins, sans-serif',
                ...
                <div className="mt-7">
                    <Table
                        ...
                        rowKey="_id"
                    />
                    <Dialog open={open} onClose={() => { setOpen(false); setSelectedBanner(null); }} className="relative z-10">
                        <DialogBackdrop
                            transition
                            ...
                            <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                                ...
                                >
                                    ...
                                    <div className="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                                        <button
                                            type="button"
                                            onClick={() => selectedBanner && handleDelete(selectedBanner._id)}
